                ...
                ScrollView(.vertical, showsIndicators: false) {
                    LazyVStack(spacing: 24, pinnedViews: [.sectionHeaders]) {
                        Section {
                            ...
                            .padding(.bottom, 48)
                            .frame(maxWidth: SettingsLayout.maxContentWidth)
...
private extension SettingsView {
    ...
    var dataManagementSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            VStack(spacing: 0) {
                NavigationLink(value: SettingsRoute.trash) {
                    dataManagementRow(
                        title: "待删区",
                        countText: "\(viewModel.pendingDeletionItems.count) 张",
                        tint: .red,
                        icon: "trash"
                    )
                }
                ...
                NavigationLink(value: SettingsRoute.skipped) {
                    dataManagementRow(
                        title: "待确认照片",
                        countText: "\(viewModel.skippedItems.count) 张",
                        tint: .purple,
    ...
    func dataManagementRow(title: String, countText: String, tint: Color, icon: String) -> some View {
        HStack {
            dataIcon(background: tint.opacity(0.12))
                .overlay(
...
struct PreferencesView: View {
    @ObservedObject var viewModel: PhotoCleanupViewModel
    @AppStorage(SettingsPreferenceKey.ignoreFavorites.rawValue) private var ignoreFavorites = true
    @AppStorage(SettingsPreferenceKey.confirmDeletion.rawValue) private var confirmDeletion = true
...
struct AdvancedOperationsView: View {
    ...
    private var clearCacheTile: some View {
        HStack(spacing: 10) {
            ...
            .padding(.horizontal, 10)
            .padding(.vertical, 5)
            .foregroundColor(viewModel.pendingDeletionItems.isEmpty ? .gray.opacity(0.6) : .red)
    ...
    private var pendingCacheDescription: String {
        ...
        return count == 0 ? "待删区当前为空" : "已暂存 \(count) 张照片"
    }
...
private enum SettingsPreferenceKey: String {
    case ignoreFavorites = "settings.ignoreFavorites"
    case confirmDeletion = "settings.confirmDeletion"
}
